...
serve(async (req) => {
  ...
  try {
    ...
    if (!RESEND_API_KEY || !SERVICE_ROLE_KEY || !SUPABASE_URL) {
      console.error("Missing required env vars for opportunity-email function");
    ...
    // Fetch subscribers that match the category (or all if subscriber has none)
    ...
    // Send individually to keep it simple
...
Apply: ${record.apply_url || "N/A"}`;
    ...
    console.error("opportunity-email error", err);
